...
getVerifyHistory = async (userid) => {
    ...
    var getVerifyHistorySql = "SELECT verifyid,transid,userid,DATE_FORMAT(verifydate,'%M %d,%Y %T') as dateFormat FROM `verification` where userid = ? and verifydate = (SELECT max(verifydate) from verification where transid like ? )";
